        ...
        async with async_playwright() as playwright:
            ...
            try:
                ...
                if args.command == 'preview':
                    report = await build_preview(context, pages=args.pages, content=args.content,
                                                 relationship=args.relationship)
                    path = save_preview(report, ROOT / '.local-data')
                    print(f"Read-only preview: {len(report['selected'])} selected; {len(report['unknown'])} unresolved among {report['candidate_count']} content-matching items.")
                    print(f"Partial Likes inventory: {report['collected_count']} items, {report['likes_collection']['pages']} pages; {report['likes_collection']['stop_reason']}.")
                    for item in report['selected']:
                        print(f"  {item['product']} | author {item['author_id'] or 'unknown'} | {item['evidence']} | {item['relationship']} | {item['url']}")
                    if report['unknown']:
                        print('Unresolved items are listed separately in the saved preview; they are never treated as not-followed.')
                    if report['following'] and not report['following']['complete']:
                        print('Following collection is incomplete; absent authors remain unknown.')
                    print(f'Saved: {path}')
                    return 0 if report['likes_collection']['stop_reason'] == 'page limit reached' else 1
...
def main():
    ...
    for name in ('login', 'following', 'preview'):
        ...
        if name == 'preview':
            command.add_argument('--pages', type=int, default=3, help='Maximum Likes pages, 1–20; not a full-history claim')
            command.add_argument('--content', choices=['all', 'reels', 'posts'], default='all')
            command.add_argument('--relationship', choices=['all', 'followed', 'not-followed', 'unknown'], default='all')
        if name == 'following':
