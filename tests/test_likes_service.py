...
from tests.test_probe_likes import payload
...
class LikesServiceTests(unittest.IsolatedAsyncioTestCase):
    async def test_live_seed_and_browser_continuation_deduplicate(self):
        ...
        page = SimpleNamespace(
            ...
            evaluate=AsyncMock(return_value={"status": 200, "body": media_body("200")}),
            close=AsyncMock(),
        )
        context = SimpleNamespace(new_page=AsyncMock(return_value=page))
        with patch(
            "insta_cleaner.likes.session_identity", AsyncMock(return_value=("1", None))
        ), patch(
            "insta_cleaner.likes.asyncio.sleep", new_callable=AsyncMock
