...
def body(pk='1', more=True, cursor='next', **extra):
    return json.dumps(dict(users=[{'pk': pk, 'username': 'example'}], status='ok',
                          has_more=more, next_max_id=cursor,
                          should_limit_list_of_followings=False,
                          hidden_following_account_count=0, **extra))
...
class FollowingTests(unittest.TestCase):
    ...
    def test_failure_reasons_exclude_raw_response(self):
        ...
        self.assertEqual(result['reasons'], ['invalid JSON or response body unavailable'])
    ...
    def test_chain_and_count(self):
        ...
        self.assertTrue(t.add(('42', 'next', True), 200, body('2', False, ''))['complete'])
        t.expected['42'] = 3
    ...
    def test_restrictions_and_missing_terminal(self):
        for changes in ({'has_more': None}, {'should_limit_list_of_followings': True},
                        {'hidden_following_account_count': 1}):
            data = json.loads(body('1', False, '')); data.update(changes)
            t = FollowingTracker()
