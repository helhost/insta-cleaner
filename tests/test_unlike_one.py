import unittest
from unlike_one import selected_item, require_same_account

class UnlikeSelectionTests(unittest.TestCase):
    def setUp(self):
        self.item = dict(code='ABC', product='clips', relationship='not-followed',
                         evidence='matched', media_id='100', author_id='42')
    ...
    def test_exact_single_selection(self):
        ...
        for rows in [[], [self.item, self.item]]:
            with self.assertRaises(ValueError): selected_item({'selected': rows}, 'ABC')

    def test_unresolved_or_wrong_content_rejected(self):
        for changes in [{'evidence': 'inferred'}, {'relationship': 'followed'},
                        {'product': 'feed'}, {'author_id': None}]:
...
class SingleClickTests(unittest.IsolatedAsyncioTestCase):
    async def test_click_once_and_reload_verify(self):
        ...
        page = SimpleNamespace(reload=AsyncMock(), url='https://www.instagram.com/p/ABC/')
        with patch('unlike_one.post_actions', AsyncMock(return_value=actions)):
