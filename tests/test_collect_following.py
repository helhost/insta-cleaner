...
from following_check import FollowingTracker
import collect_following
...
class CollectorTests(unittest.IsolatedAsyncioTestCase):
    async def run_collection(self, responses):
        ...
        seed = SimpleNamespace(url='https://www.instagram.com/api/v1/friendships/42/following/?count=12',
                               request=SimpleNamespace(all_headers=AsyncMock(return_value={'x-ig-app-id': 'test'})))
        replies = [SimpleNamespace(status=status, text=AsyncMock(return_value=data), dispose=AsyncMock())
                   for status, data in responses]
        context = SimpleNamespace(request=SimpleNamespace(get=AsyncMock(side_effect=replies)))
        with tempfile.TemporaryDirectory() as directory:
            with patch.object(collect_following, 'OUTPUT', Path(directory)), patch.object(collect_following.asyncio, 'sleep', new_callable=AsyncMock), contextlib.redirect_stdout(io.StringIO()):
    ...
    async def test_stops_without_new_members(self):
        snapshot, get = await self.run_collection([(200, body('1', True, str(i))) for i in range(3)])
        self.assertFalse(snapshot['complete'])
    ...
    async def test_direct_start_uses_session_owner_and_requested_size(self):
        ...
        reply = SimpleNamespace(status=200, text=AsyncMock(return_value=body('1', False, '')),
                                dispose=AsyncMock())
        context = SimpleNamespace(cookies=AsyncMock(return_value=cookies),
                                  request=SimpleNamespace(get=AsyncMock(return_value=reply)))
        with tempfile.TemporaryDirectory() as directory:
            with patch.object(collect_following, 'OUTPUT', Path(directory)), contextlib.redirect_stdout(io.StringIO()) as output:
                await collect_following.collect_signed_in(context, tracker, 100)
            saved = next(Path(directory).glob('*.json')).read_text()
        self.assertEqual(context.request.get.call_args.args[0],
    ...
    async def test_missing_session_does_not_request_an_account(self):
        context = SimpleNamespace(cookies=AsyncMock(return_value=[{'name': 'ds_user_id', 'value': '42'}]),
                                  request=SimpleNamespace(get=AsyncMock()))
        with patch.object(collect_following.asyncio, 'sleep', new_callable=AsyncMock), contextlib.redirect_stdout(io.StringIO()):
    ...
    async def test_headless_missing_login_exits_without_waiting(self):
        context = SimpleNamespace(cookies=AsyncMock(return_value=[]),
                                  request=SimpleNamespace(get=AsyncMock()))
        with patch.object(collect_following.asyncio, 'sleep', new_callable=AsyncMock) as sleep, contextlib.redirect_stdout(io.StringIO()) as output:
            await collect_following.collect_signed_in(context, FollowingTracker(), wait_for_login=False)
