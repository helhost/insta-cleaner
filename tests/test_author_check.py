...
class AuthorTests(unittest.TestCase):
    ...
    def test_route_instance_values_not_parameter_schema(self):
        result = {
            "route_match_infos": [
                {
                    "instanceParams": {"shortcode": "ABC"},
                    "routeParams": {"shortcode": {"coercibleType": "string"}},
                }
            ],
            "exports": {
                "rootView": {"props": {"media_id": "100", "media_owner_id": "42"}}
            },
        }
        self.assertEqual(
            metadata(json.dumps({"payload": {"result": result}})),
            [("100", "ABC", {"42"})],
        )
    ...
    def test_bulk_route_results_keep_authors_scoped(self):
        def route(code, pk, owner):
            return {
                "route_match_infos": [{"instanceParams": {"shortcode": code}}],
                "exports": {
                    "hostableView": {"props": {"media_id": pk, "media_owner_id": owner}}
                },
            }
        ...
        body = json.dumps(
            {
                "payloads": {
                    "first": {"result": route("ABC", "100", "42")},
                    "second": {"result": route("DEF", "200", "43")},
                }
            }
        )
        self.assertCountEqual(
            metadata(body), [("100", "ABC", {"42"}), ("200", "DEF", {"43"})]
        )
        self.assertEqual(metadata(body, "ABC"), [("100", "ABC", {"42"})])
        self.assertTrue(
            metadata_endpoint("https://www.instagram.com/ajax/route-definition/")
        )
    ...
    def test_initial_document_embedded_json(self):
        ...
        html = (
            '<html><script>throw "must not execute";</script><script type="application/json">'
            + json.dumps({"payload": json.dumps({"data": node})})
            + "</script></html>"
        )
    ...
    def test_diagnostics_are_counts_not_private_values(self):
        rows, counts = inspect_metadata(
            json.dumps(
                {
                    "secret": "SECRET",
                    "data": {"code": "ABC", "user": {"username": "PRIVATE"}},
                }
            )
        )
    ...
    def test_graphql_owner_and_errors(self):
        node = {"id": "100", "shortcode": "ABC", "owner": {"id": "42"}}
        self.assertEqual(
            metadata(json.dumps({"data": node}), "ABC"), [("100", "ABC", {"42"})]
        )
        self.assertEqual(
            metadata(json.dumps({"data": node, "errors": ["failure"]}), "ABC"), []
        )
    ...
    def test_exact_join_reverse_order_and_duplicate(self):
        ...
        self.assertEqual(
            t.add_likes({"100_42": ("ABC", "clips", "2")}), [(1, "matched")]
        )
        self.assertEqual(t.add_details([("100", "ABC", {"42"})]), [])
    ...
    def test_endpoint_and_page_scope(self):
        ...
        self.assertTrue(
            metadata_endpoint("https://www.instagram.com/api/v1/media/100/info/")
        )
        self.assertFalse(
            metadata_endpoint("https://www.instagram.com/api/v1/media/100/delete/")
        )
