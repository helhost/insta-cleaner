...
from insta_cleaner.following import (
    collect_following,
    FollowingSnapshot,
    filter_by_following,
)
...
class ServiceTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_restricted_terminal_is_incomplete(self):
        ...
        snapshot = await collect_following(
            context_for([(200, json.dumps(data))]), delay=0
        )
        self.assertFalse(snapshot.complete)
    ...
    def test_filters_require_author_evidence_and_same_account(self):
        snapshot = FollowingSnapshot(
            "42", "now", {"1": "example"}, True, 1, "final page reached"
        )
        items = [
            {"author_id": "1", "evidence": "matched"},
        ...
        self.assertEqual(
            filter_by_following(
                items, snapshot, account_id="42", relationship="followed"
            ),
            items[:1],
        )
        self.assertEqual(
            filter_by_following(
                items, snapshot, account_id="42", relationship="not-followed"
            ),
            items[1:2],
        )
        self.assertEqual(
            filter_by_following(
                items, snapshot, account_id="42", relationship="unknown"
            ),
            items[2:],
        )
        self.assertEqual(
            filter_by_following(
                items, snapshot, account_id="43", relationship="unknown"
            ),
            items,
        )
