import json
import unittest
from tools.probe_likes import next_params, envelope_error
...
def payload(cursors):
    ...
    for cursor in cursors:
        values = ", ".join(
            json.dumps(x) for x in ["9", '{"test":"escaped value"}', cursor, "1", "2"]
        )
        expressions.append(
            "(bk.action.bloks.AsyncActionWithDataManifest, "
            '"com.instagram.privacy.activity_center.liked_next", '
            '(bk.action.map.Make, (bk.action.array.Make, "page_size", "activity_center_params", '
            '"cursor", "container_id", "element_id"), (bk.action.array.Make, '
            + values
            + ")))"
        )
...
class PaginationTests(unittest.TestCase):
    def test_extracts_escaped_json_without_evaluation(self):
        ...
        self.assertEqual(
            json.loads(params["activity_center_params"]), {"test": "escaped value"}
        )
...
class EnvelopeTests(unittest.TestCase):
    def test_server_error_is_reported_without_private_details(self):
        body = json.dumps(
            {"payload": None, "error": 1357054, "errorDescription": "PRIVATE"}
        )
        self.assertEqual(
            envelope_error(body), "unsupported page envelope; server error 1357054"
        )

    def test_non_object_and_non_numeric_errors_are_redacted(self):
        for body in ["null", "[]", '{"error":"PRIVATE"}']:
            self.assertEqual(
                envelope_error(body), "unsupported page envelope; server error unknown"
            )
...
class AuthorSampleTests(unittest.IsolatedAsyncioTestCase):
    async def test_sample_joins_exact_identity_only(self):
        ...
        from tools.author_check import AuthorTracker
        from tools.probe_likes import check_authors

        tracker = AuthorTracker()
        tracker.add_likes(
            {"100_42": ("ABC", "clips", "2"), "200_43": ("DEF", "feed", "1")}
        )
        data = json.dumps(
            {
                "items": [
        ...
        page = SimpleNamespace(
            ...
            goto=AsyncMock(),
            content=AsyncMock(return_value=data),
            close=AsyncMock(),
        )
        context = SimpleNamespace(new_page=AsyncMock(return_value=page))
        with patch(
            "tools.probe_likes.asyncio.sleep", new_callable=AsyncMock
        ), contextlib.redirect_stdout(io.StringIO()):
            await check_authors(context, tracker, 1)
        self.assertEqual(
            [row["evidence"] for row in tracker.preview()], ["matched", "inferred"]
        )
...
class AutomaticProbeTests(unittest.IsolatedAsyncioTestCase):
    async def test_opens_likes_directly(self):
        ...
        from tools.probe_likes import open_likes
    ...
    async def test_headless_requires_login_before_navigation(self):
        from types import SimpleNamespace
        from unittest.mock import AsyncMock
        from tools.probe_likes import open_likes, LoginRequired
    ...
    async def test_login_redirect_is_actionable(self):
        ...
        page = SimpleNamespace(
            goto=AsyncMock(), url="https://www.instagram.com/accounts/login/"
        )
    ...
    async def test_author_response_wakes_wait_and_missing_metadata_times_out(self):
        ...
        for deliver in (True, False):
            ...
            response = SimpleNamespace(
                status=200,
                text=AsyncMock(return_value=data),
            ...
            page = SimpleNamespace(
                on=lambda name, fn: handlers.update({name: fn}),
                remove_listener=Mock(),
                goto=AsyncMock(),
                content=content,
                close=AsyncMock(),
            ...
            self.assertEqual(
                tracker.preview()[0]["evidence"], "matched" if deliver else "inferred"
            )
