...
def following(complete=True, account="1"):
    return FollowingSnapshot(
        account, "now", {"42": "example"}, complete, 1, "final page reached"
    )
...
class SelectionTests(unittest.TestCase):
    ...
    def test_other_account_cannot_classify_membership(self):
        selected, unknown = select_items(
            items(), account_id="2", following=following(), relationship="followed"
        )
        self.assertEqual(selected, [])
...
class WorkflowTests(unittest.IsolatedAsyncioTestCase):
    async def test_following_is_loaded_only_when_needed(self):
        for relationship in ["all", "not-followed"]:
            ...
            tracker.add_likes(
                {"100_43": ("ABC", "clips", "2"), "200_42": ("DEF", "feed", "1")}
            )
            ...
            with patch(
                "insta_cleaner.preview.session_identity",
                AsyncMock(return_value=("1", None)),
            ), patch(
                "insta_cleaner.preview.collect_likes",
                AsyncMock(
                    return_value=(
                        tracker,
                        {
                ...
                "insta_cleaner.preview.check_authors", AsyncMock(side_effect=resolve)
            ), patch(
                "insta_cleaner.preview.collect_following",
                AsyncMock(return_value=following()),
            ) as collect, contextlib.redirect_stdout(
                io.StringIO()
            ):
                report = await build_preview(
                    None, pages=1, content="reels", relationship=relationship
                )
    ...
    async def test_account_switch_rejects_preview(self):
        with patch(
            ...
            "insta_cleaner.preview.collect_likes",
            AsyncMock(return_value=(AuthorTracker(), {"account_id": "2"})),
        ):
