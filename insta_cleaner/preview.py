...
def select_items(
    items, *, account_id, following=None, content="all", relationship="all"
):
    if content not in {"all", "reels", "posts"} or relationship not in {
        "all",
        "followed",
        "not-followed",
        "unknown",
    ...
    for item in items:
        if not content_matches(item, content):
            continue
        explicit = item.get("evidence") in {"matched", "mismatched"} and bool(
            item.get("author_id")
        )
        membership = "unknown"
        ...
        row = {
            **item,
            "relationship": membership if following else "not-checked",
        ...
        if not explicit or (following is not None and membership == "unknown"):
            unknown.append(row)
        if relationship == "all" or (
            following is not None and membership == relationship
        ):
            selected.append(row)
...
async def build_preview(context, *, pages=3, content="all", relationship="all"):
    ...
    if collection["account_id"] != account:
        raise LoginRequired(
            "Session changed during collection; discard this preview and rerun."
        )
    candidates = [item for item in tracker.preview() if content_matches(item, content)]
    if candidates:
        print(
            f"Checking authors for {len(candidates)} collected {content} items…",
            flush=True,
        )
    ...
    return {
        ...
        "likes_collection": collection,
        "collected_count": len(tracker.likes),
        ...
        "following": (
            ...
            if following is None
                ...
                "count": len(following.accounts),
                "stop_reason": following.stop_reason,
            }
        ),
        "selected": selected,
        "unknown": unknown,
