...
def following_request(url):
    try:
        ...
        if parts.scheme != "https" or parts.hostname not in {
            ...
            return None
        match = re.fullmatch(
            r"/api/v1/friendships/([1-9][0-9]*)/following/", parts.path
        )
        if not match:
            return None
        query = parse_qs(parts.query, keep_blank_values=True)
        # Search and other filters are not complete following-list traversals.
        valid = set(query) <= {"count", "max_id"} and all(
...
class FollowingTracker:
    ...
    def add(self, request, status, body):
        ...
        state = self.accounts.setdefault(
            account, {"pages": {}, "failed": False, "reasons": set()}
        )
        ...
        try:
            ...
            if status != 200:
                reason = (
                    f"HTTP {status}" if status else "response body could not be read"
                )
                raise ValueError()
            ...
            if bool(next_cursor) != more:
                ...
                raise ValueError()
            unrestricted = (
                data.get("should_limit_list_of_followings") is False
            ...
            if old is not None and (set(old[0]) != set(records) or old[1:] != page[1:]):
                ...
                state["reasons"].add(
                    "repeated cursor returned different members, pagination, or restrictions"
                )
    ...
    def show(self, account):
        ...
        expected = (
            str(result["expected"]) if result["expected"] is not None else "unknown"
        )
        state = "complete observed traversal" if result["complete"] else "INCOMPLETE"
        print(
            f"Following list #{label}: {result['users']} unique accounts; profile count {expected}; "
            f"{result['pages']} pages; {state}.",
            flush=True,
        )
        if result["failed"]:
        ...
        print(
            "  List owner is not yet verified as the signed-in account; no negative membership filter is enabled."
        )
