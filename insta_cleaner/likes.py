...
def next_params(body):
    ...
    if len(found) != 1 or not found[0]["cursor"]:
        raise ValueError(
            "No single supported next-page instruction; completion is unverified"
        )
    return found[0]
...
async def check_authors(context, tracker, limit, timeout=8, *, items=None):
    ...
    def observe(response):
        if (
            metadata_endpoint(response.url)
    ...
    try:
        for item in (tracker.preview() if items is None else items)[:limit]:
            ...
            try:
                ...
                state = next(
                    row["evidence"]
                    for row in tracker.preview()
                    if (row["media_id"], row["code"]) == key
                )
                print(f"  Author sample #{item['index']}: {state}.", flush=True)
            except Exception:
                print(
                    f"  Author sample #{item['index']}: unavailable; retaining existing evidence.",
                    flush=True,
                )
    finally:
...
async def open_likes(context, page, ready, *, manual=False, headless=False):
    ...
    if manual:
        print(
            "Open Your activity → Likes. Collection starts automatically.", flush=True
        )
    else:
    ...
    target = (
        ...
        if manual
        else "https://www.instagram.com/your_activity/interactions/likes/"
    )
    await page.goto(target, wait_until="domcontentloaded", timeout=30000)
    if "/accounts/login" in page.url or "/challenge/" in page.url:
        raise LoginRequired(
            "Login or verification required. Run: python3 instagram.py login"
        )
...
async def collect_likes(context, *, pages=3):
    ...
    if (await session_identity(context))[0] != account:
        raise LoginRequired(
            "Session changed during collection; rerun with one account."
        )
    return tracker, {
        "account_id": account,
        "pages": batches,
