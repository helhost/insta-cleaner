...
async def session_identity(context):
    ...
    if len(ids) != 1 or not any(c['name'] == 'sessionid' and c['value'] for c in cookies):
...
async def collect_following(context, *, page_size=100, delay=1.5, progress=None):
    ...
    for page in range(1000):
        ...
        if result['failed']:
            ...
            break
        records, next_cursor, more, unrestricted = tracker.accounts[account]['pages'][cursor]
        before = len(users)
    ...
    return FollowingSnapshot(account, datetime.now(timezone.utc).isoformat(), users,
                             complete, pages, reason)
...
def filter_by_following(items, snapshot, *, account_id, relationship):
    ...
    return [item for item in items if (
        ...
        if item.get('evidence') in {'matched', 'mismatched'} else 'unknown'
    ) == relationship]
