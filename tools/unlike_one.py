...
def selected_item(report, code):
    ...
    item = rows[0]
    if (item.get('product') != 'clips' or item.get('relationship') != 'not-followed'
        ...
        raise ValueError('This experiment requires a reel with an explicit, not-followed author.')
    return item
...
async def click_and_verify(page, unlike, like, code):
    ...
    await actions.locator('svg[aria-label="Like"]').wait_for(state='visible', timeout=15000)
...
async def run(args):
    ...
    journal = {'code': args.code, 'media_id': item['media_id'], 'account_id': expected,
               'execute': args.execute, 'status': 'checking', 'click_attempted': False}
    try:
        async with async_playwright() as p:
            context = await p.chromium.launch_persistent_context(str(ROOT / '.browser-profile'),
                channel='chrome', headless=True, chromium_sandbox=True)
            try:
                require_same_account(expected, (await session_identity(context))[0])
                print('Refreshing Following and author evidence for the single selected reel…', flush=True)
                following = await collect_following(context)
                require_same_account(expected, following.list_owner_id)
                if following.membership(item['author_id'], account_id=expected) != 'not-followed':
                    raise ValueError('Fresh Following data does not establish that this author is not followed.')
                tracker = AuthorTracker()
                tracker.add_likes({f"{item['media_id']}_{item['author_id']}": (args.code, 'clips', '2')})
                await check_authors(context, tracker, 1)
                fresh = tracker.preview()[0]
                if fresh['evidence'] != 'matched' or fresh['author_id'] != item['author_id']:
                    raise ValueError('Fresh author metadata does not confirm the selected author.')
                ...
                if await unlike.count() != 1 or await like.count() != 0:
                    raise ValueError('The selected post is not unambiguously liked. No click sent.')
                require_same_account(expected, (await session_identity(context))[0])
                print(f'Checked {args.code}: correct account, explicit author, not followed, currently liked.', flush=True)
                if not args.execute:
                ...
                print(f'Unliked {args.code}; verified after reloading. No other items touched.', flush=True)
                return 0
            finally:
                await context.close()
    except Exception as error:
        # Never expose browser exception logs that might contain session data.
        print('Stopped: ' + (str(error) if type(error) is ValueError else type(error).__name__), flush=True)
        if not journal['click_attempted']:
            journal['status'] = 'stopped_before_click'
        if journal['click_attempted']:
            print('A click may have reached Instagram. Check this item manually; do not retry blindly.')
        return 1
    finally:
...
def save_journal(journal):
    ...
    path = directory / ('unlike-test-' + datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S%fZ') + '.json')
...
def main():
    ...
    parser.add_argument('--execute', action='store_true', help='Unlike this one checked item and verify after reload')
