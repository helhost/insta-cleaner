...
from tools.inspect_har import PREFIX, response_records
from tools.author_check import (
    AuthorTracker,
    inspect_metadata,
    metadata_endpoint,
    post_code,
...
from tools.collect_following import collect, collect_signed_in
...
def summarize(action, filter_state, author_ids, status, body):
    ...
    if warning:
        result["note"] = (
            "No supported media records found; this does not mean the history is empty."
        )
        return result
...
class Reporter:
    ...
    def show(self, result):
        ...
        print(
            f"\nBatch {self.batches}: {result['action']} — {result['items']} items",
            flush=True,
        )
        if result.get("products"):
            print(
                "  Types: "
                + ", ".join(f"{k}: {v}" for k, v in sorted(result["products"].items()))
            )
        if result["comparison"] in {"consistent", "mismatch"}:
            print(f"  Author filter: {result['selected_authors']} selected ID(s).")
            print(
                f"  Candidate author IDs matching that filter: {result['matching_items']}/{result['items']}."
            )
            print(
                "  "
                + (
                    "Consistent with the ID-suffix hypothesis; not independently verified."
                    if result["comparison"] == "consistent"
                    else "MISMATCH: do not rely on ID suffixes for author filtering yet."
                )
            )
        elif result["filter"] == "unfiltered":
            print(
                "  Open a liked post normally to check its author; no author filter is needed."
            )
        if "note" in result:
            print("  " + result["note"])
        sys.stdout.flush()
    ...
    def finish(self):
        ...
        print(
            f"Filter comparisons: {self.comparisons['consistent']} consistent, "
            f"{self.comparisons['mismatch']} mismatched."
        )
        if not self.comparisons["consistent"] and not self.comparisons["mismatch"]:
            print(
                "No request-filter comparison completed; see the post author checks below."
            )
        print(
            "This does not verify following status, account identity, or complete history."
        )
...
def show_preview(tracker, author_id=None):
    ...
    if author_id:
        print(
            "Author-ID filter applied; inferred matches included, ambiguous identities excluded."
        )
    ...
    for row in rows:
        ...
        }.get(row["product"], "unknown")
        print(
            f"{row['index']:<4} {content:<21} {row['author_id'] or 'unknown':<21} "
            f"{row['evidence']:<14} https://www.instagram.com/p/{row['code']}/"
        )
    print(
        "matched = explicit owner agrees; inferred = ID suffix only; "
    ...
    print(
        "Private activity: review before sharing. Nothing selected or removed; history may be incomplete or stale."
    )
...
async def observe(
    ...
    try:
        ...
        print(
            "Install the browser dependency: python3 -m pip install -r requirements.txt",
            file=sys.stderr,
        )
        return 2
    ...
    def show_checks(changes):
        for index, state in changes:
            ...
            print(
                f"Author check for captured item #{index}: {messages[state]}",
                flush=True,
            )
    ...
    try:
        ...
        async with async_playwright() as playwright:
                ...
                try:
                    ...
                    if follow_request:
                        ...
                        if (
                            auto_following
                            and not auto_open
                            and not collection_started
                            and not follow_request[1]
                            and follow_request[2]
                        ):
                            collection_started = True
                            job = asyncio.create_task(
                                collect(context, response, following, page_size)
                            )
                            tasks.add(job)
                    ...
                    if action:
                        reporter.show(
                            summarize(action, state, authors, response.status, body)
                        )
                        if response.status == 200:
                            try:
                                records, _ = response_records(
                        ...
                        try:
                            ...
                            rows = [
                                row
                                for row in rows
                                if (row[0], row[1]) in tracker.likes or row[1] == code
                            ]
                            ...
                            if not rows and code:
                                ...
                                print(
                                    f"Post diagnostic: HTTP {response.status}; {counts or 'body not inspected'}.",
                                    flush=True,
                                )
                            elif rows and not any(
                                (pk, item_code) in tracker.likes
                                for pk, item_code, _ in rows
                            ):
                                print(
                                    "Post metadata observed; first load this item in Likes to compare it.",
                                    flush=True,
                                )
                        except (ValueError, TypeError, AttributeError):
                            print(
                                "Post response format unsupported; no raw data printed.",
                                flush=True,
                            )
                except BrowserError:
                    follow_request = following_request(response.url)
                    if follow_request:
                        ...
                        following.show(follow_request[0])
                    print(
                        "A response could not be read, possibly because the page closed."
                    )
            ...
            def on_response(response):
                ...
                if action is None and not (
                    is_metadata
                    or (is_document and code)
                    or following_request(response.url)
                ):
                    return
                state, authors = selected_authors(response.request.post_data)
                task = asyncio.create_task(
                    handle(response, action, state, authors, code)
                )
                tasks.add(task)
            ...
            if auto_following:
                ...
                print(
                    "Loading your Following list directly."
                    if auto_open
                    else "Open your own profile → Following once. No scrolling needed."
                )
                print("Saves account IDs and usernames locally. Removes nothing.")
            else:
                print(
                    "Browser observer ready. Log in manually, then open Your activity → Likes."
                )
                print(
                    "To collect Following manually: open your own profile → Following and scroll inside the list."
                )
                print(
                    "Wait for a Likes batch, then open ONE liked post normally (not in Select mode)."
                )
                print(
                    "Wait for an Author check. Try another post or refresh the opened post if none appears."
                )
                print(
                    "The observer does not click, replay requests, or remove anything."
                )
            if preview:
                print(
                    "A preview with author IDs and post links will appear when you close the browser."
                )
            if headless:
                print(
                    "Running without a browser window; exits after saving the snapshot.",
                    flush=True,
                )
            else:
                print(
                    "Your own browser clicks still work normally. Avoid deletion during this experiment."
                )
                print(
                    "Close all observer browser windows or press Ctrl+C here to finish.",
                    flush=True,
                )
            ...
            try:
                ...
                print(
                    "Initial navigation did not finish. You can open Instagram manually in this window."
                )
            if auto_open:
                try:
                    await collect_signed_in(
                        context, following, page_size, wait_for_login=not headless
                    )
                except BrowserError:
                    print(
                        "Could not load Following directly. Check the browser for a login challenge, then rerun."
                    )
            if headless:
        ...
        print(
            "Could not run the browser. Install Chromium with 'python3 -m playwright install chromium', "
            "or use '--channel chrome' for installed Chrome. Close other observer instances first.",
            file=sys.stderr,
        )
        return 2
    finally:
        ...
        print(
            f"Post author checks: {counts['matched']} matched, {counts['mismatched']} mismatched, "
            f"{counts['conflicting']} conflicting; {len(tracker.likes) - len(tracker.reported)} unchecked items."
        )
        print(
            "Matches validate only the observed items, not following status or other posts."
        )
...
def main():
    ...
    parser.add_argument(
        "--preview",
        action="store_true",
        help="Print captured items, author IDs, and post links when stopping",
    )
    parser.add_argument(
        "--collect-following",
        action="store_true",
        help="Automatically fetch remaining Following pages and export usernames locally after you open the list",
    )
    parser.add_argument(
        "--author-id",
        help="Filter the preview to a numeric author ID (includes inferred matches)",
    )
    parser.add_argument(
        "--page-size",
        type=int,
        default=50,
        help="Requested accounts per page (default 50; Instagram may return fewer)",
    )
    parser.add_argument(
        "--auto-open-following",
        action="store_true",
        help="Fetch the saved session account’s Following list directly; implies --collect-following",
    )
    parser.add_argument(
        "--headless",
        action="store_true",
        help="Hide the browser and exit after collection; requires --auto-open-following and a saved login",
    )
    ...
    try:
        return asyncio.run(
            observe(
                args.channel,
                args.preview or args.author_id is not None,
                args.author_id,
                args.collect_following or args.auto_open_following,
                args.page_size,
                args.auto_open_following,
                args.headless,
            )
        )
