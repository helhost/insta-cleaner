...
from following_check import following_request
...
def export_snapshot(tracker, account):
    ...
    result = {'schema_version': 1, 'observed_at': now.isoformat(), 'list_owner_id': account,
              'session_identity_verified': False, **tracker.summary(account),
              'accounts': [{'id': pk, 'username': name} for pk, name in sorted(users.items())]}
...
async def collect(context, seed_response, tracker, page_size=50):
    ...
    seen = set()
    print('Automatically collecting Following pages. No follow/unfollow actions will be sent.', flush=True)
    try:
        raw_headers = await seed_response.request.all_headers()
        headers = {k: v for k, v in raw_headers.items() if k.lower() in
                   {'x-ig-app-id', 'x-asbd-id', 'x-csrftoken', 'x-requested-with', 'accept'}}
        stalled = 0
        for _ in range(1000):
            state = tracker.accounts[account]
            if state['failed']:
                print('Automatic collection stopped: snapshot has a failed or changed page.')
                break
            cursor = ''
            traversed = set()
            ...
            if cursor in state['pages']:
                if not unrestricted:
                    print('Automatic collection stopped: Instagram marked this list restricted.')
                elif not more:
                    print('Automatic collection finished: Instagram returned its final page.')
                    summary = tracker.summary(account)
                    if summary['expected'] is not None and summary['users'] != summary['expected']:
                        print('The returned list differs from the profile count; keeping the snapshot incomplete.')
                else:
                    print('Automatic collection stopped: Instagram repeated a pagination cursor.')
                break
            if cursor in seen:
                break
            seen.add(cursor)
            params['max_id'] = cursor
            next_url = urlunsplit((url.scheme, url.netloc, url.path, urlencode(params), ''))
            await asyncio.sleep(1.5)
            before = tracker.summary(account)['users']
            response = await context.request.get(next_url, headers=headers, timeout=30000, max_redirects=0)
            try:
            ...
            if stalled >= 3:
                print('Automatic collection stopped after three pages with no new accounts.')
                break
        else:
            print('Automatic collection stopped at the page limit; snapshot may be incomplete.')
    except asyncio.CancelledError:
        print('Automatic collection interrupted; exporting the partial snapshot.')
        raise
    except Exception:
        tracker.add((account, '__failed_request__', True), 0, '')
        print('Automatic collection stopped after a request error. No response details printed.')
    finally:
...
async def collect_signed_in(context, tracker, page_size=50, wait_for_login=True):
    ...
    for _ in range(120 if wait_for_login else 1):
        ...
        ids = {c['value'] for c in cookies if c['name'] == 'ds_user_id'
               and re.fullmatch(r'[1-9][0-9]*', c['value'])}
        ...
        if not wait_for_login:
            ...
            return
        if _ == 0:
            print('Waiting for login. Once you log in, Following collection starts automatically.', flush=True)
        await asyncio.sleep(1)
    if account is None:
        print('No signed-in session found after two minutes. Log in and rerun this command.')
        return
    ...
    print('Loading Following directly for the saved session account. No navigation or scrolling needed.', flush=True)
